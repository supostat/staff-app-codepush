...
class AblyService {
  constructor(ablyRealtime, personalChannel, presenceChannel) {
    ...
    this.deactivate = () => new Promise((resolve, reject) => {
      ensureActive();
...
export function createAblyService(options) {
  ...
  const onGetTokenFailed = options.onGetTokenFailed || function () {};
  ...
  return AppManager.getBaseUrl().then((baseURL) => {
    ...
    // private stuff
    const personalChannel = ablyRealtime.channels.get(personalChannelName);
    const presenceChannel = ablyRealtime.channels.get(presenceChannelName);
